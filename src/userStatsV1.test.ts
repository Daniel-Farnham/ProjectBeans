import { FORBIDDEN, getRequest, channelIdExists, getUidFromToken } from './other';
import {
  clearV1, authRegisterV1, dmCreateV1,
  channelsCreateV1, messageSendV1, channelJoinV1, messageSendDmV1, userStatsV1, channelLeaveV1
...
describe('Testing userStatsV1 success handling', () => {
  test('Testing userStatsV1 returns correct output (simple)', () => {
    ...
    expect(result).toMatchObject({
      userStats: {
        channelsJoined: [{ numChannelsJoined: 0, timeStamp: timeSent }, { numChannelsJoined: 1, timeStamp: timeSent }],
    		dmsJoined: [{ numDmsJoined: 0, timeStamp: timeSent }],
    		messagesSent: [{ MessagesSent: 0, timeStamp: timeSent }, { MessagesSent: 1, timeStamp: timeSent2 }],
    		involvementRate: 1,
      }
    });
  });
  test('Testing userStatsV1 returns correct output (complex)', () => {
    ...
    expect(result).toMatchObject({
      userStats: {
        channelsJoined: [{ numChannelsJoined: 0, timeStamp: time }, { numChannelsJoined: 1, timeStamp: time },
          { numChannelsJoined: 2, timeStamp: time2 }],
    		dmsJoined: [{ numDmsJoined: 0, timeStamp: time }, { numDmsJoined: 1, timeStamp: time3 }],
    		messagesSent: [{ MessagesSent: 0, timeStamp: time }, { MessagesSent: 1, timeStamp: time4 }, { MessagesSent: 2, timeStamp: time5 },
  ...
  test('User joins 2 channels and leaves 1', () => {
    const user = authRegisterV1('jackblack@gmail.com', 'password', 'Jack', 'Black');
    channelsCreateV1(user.token, 'Channel1', true);
    const time = Math.floor((new Date()).getTime() / 1000);
